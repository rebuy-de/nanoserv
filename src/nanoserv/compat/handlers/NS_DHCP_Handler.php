 ...
 * @subpackage Handlers
 */
class NS_DHCP_Exception extends NS_Exception {
...
class NS_DHCP_Message {
	...
	public function __construct($data = NULL) {
		...
		if (isset($data)) {
			...
			$this->options = NS_DHCP_Options::Decode(substr($data, 236));

		} else {

			$this->options = new NS_DHCP_Options();
 ...
 * @subpackage Handlers
 */
abstract class NS_DHCP_Handler extends NS_Datagram_Handler {

	public function on_Read($from, $data) {

		$this->on_DHCP_Message($from, NS_DHCP_Message::Decode($data));
	 ...
	 */
	abstract public function on_DHCP_Message($from, NS_DHCP_Message $msg);
