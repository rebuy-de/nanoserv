...
interface I_Write_Buffer {
	 ...
	 * @param int length
	 */
	public function __construct(Socket $socket, $data, $callback = false, $length = NULL);
...
abstract class Connection_Handler extends Handler {
	...
	public function Write_Stream($stream, $callback=false) {
		...
		return Core::New_Stream_Write_Buffer($this->socket, $data, $callback);
		...
		do {
			...
			if (($rfd || $wfd) && (@stream_select($rfd, $wfd, $efd, $tv_sec, $tv_usec))) {
				...
				foreach ($rfd as $act_rfd) {
					...
					} else if ($handler instanceof Connection_Handler) {
