 ...
 * @since 2.0
 */
abstract class NS_Exception extends Exception {
		...
		if (($proto === "udp") || ($proto === "unix")) {
			...
			if ($proto != "tcp") switch ($proto) {
				...
				if (defined($cname = "STREAM_CRYPTO_METHOD_".strtoupper($proto)."_SERVER")) {
					...
					throw new NS_Server_Exception("unknown transport/crypto type '{$proto}'");
...
interface NS_I_Write_Buffer {
	 ...
	 * @param int length
	 */
	public function __construct(NS_Socket $socket, $data, $callback = false, $length = NULL);
...
abstract class NS_Connection_Handler extends NS_Handler {
	...
	public function Write_Stream($stream, $callback=false) {
		...
		return Nanoserv::New_Stream_Write_Buffer($this->socket, $data, $callback);
...
final class Nanoserv {
	 ...
	 * @since 0.9
	 */
	static public function New_Static_Write_Buffer(NS_Socket $socket, $data, $callback=false) {

		$wb = new NS_Static_Write_Buffer($socket, $data, $callback);
	 ...
	 * @since 0.9
	 */
	static public function New_Stream_Write_Buffer(NS_Socket $socket, $data, $callback=false) {
			...
			foreach (self::$connections as $c) {
				...
				if ($c->socket->pending_crypto) {
					...
					if ($cr === true) {
						...
						$c->on_Connect_Fail(NS_Connection_Handler::FAIL_CRYPTO);
					...
					if (!isset($handler)) {
						...
						if ($handler->socket->pending_crypto) {
							...
							if ($cr === true) {
								...
								$handler->on_Connect_Fail(NS_Connection_Handler::FAIL_CRYPTO);
				...
				foreach ($wfd as $act_wfd) {
					...
					if (!isset($handler)) {
						...
						if ($handler->socket->Eof()) {
							...
							$handler->on_Connect_Fail(NS_Connection_Handler::FAIL_CONNREFUSED);
							self::Free_Connection($handler);
