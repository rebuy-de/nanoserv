...
install-documentation:
	...
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/ChangeLog $(INSTALL_DIR_DOC)
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/README $(INSTALL_DIR_DOC)
	if [ -d $(DOC_DIR) ]; then mkdir -p $(INSTALL_DIR_PHPDOC); cp -R $(DOC_DIR)/* $(INSTALL_DIR_PHPDOC); fi

install-examples:
	$(INSTALL_BIN) -d $(INSTALL_DIR_EXAMPLES)
	$(INSTALL_BIN) --mode=0755 $(MAIN_DIR)/examples/*.php $(INSTALL_DIR_EXAMPLES)
...
install-main:
	...
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/nanoserv.php $(INSTALL_DIR_MAIN)
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/*.php $(INSTALL_DIR_MAIN)/handlers
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/HTTP/*.php $(INSTALL_DIR_MAIN)/handlers/HTTP
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/SMTP/*.php $(INSTALL_DIR_MAIN)/handlers/SMTP
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/Syslog/*.php $(INSTALL_DIR_MAIN)/handlers/Syslog
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/Telnet/*.php $(INSTALL_DIR_MAIN)/handlers/Telnet
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/DHCP/*.php $(INSTALL_DIR_MAIN)/handlers/DHCP
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/HTTP/XML_RPC/*.php $(INSTALL_DIR_MAIN)/handlers/HTTP/XML_RPC
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/HTTP/SOAP/*.php $(INSTALL_DIR_MAIN)/handlers/HTTP/SOAP
	$(INSTALL_BIN) --mode=0644 $(MAIN_DIR)/handlers/HTTP/JSON_RPC/*.php $(INSTALL_DIR_MAIN)/handlers/HTTP/JSON_RPC

install-compat:
	$(INSTALL_BIN) -d $(INSTALL_DIR_COMPAT)
	$(INSTALL_BIN) -d $(INSTALL_DIR_COMPAT)/handlers
	$(INSTALL_BIN) --mode=0644 $(COMPAT_DIR)/nanoserv.php $(INSTALL_DIR_COMPAT)
	$(INSTALL_BIN) --mode=0644 $(COMPAT_DIR)/handlers/*.php $(INSTALL_DIR_COMPAT)/handlers
